---
title: MoonYuki subset
description: Line-oriented visual-novel DSL subset implemented in Phases 1–4
---
...
>
> Adapted from `docs/moon-yuki-subset.md`. Cross-links to unmigrated topics still
> point at the repository `docs/` tree.
...
`std_ui` / `ui_package` — see repo `docs/ui-moonbit.md`). This document
...
YukimiScript bytecode compatibility, `trans.dissolve` as a MoonYuki-only feature
doc (engine dissolve is Q2 host-commands territory), backlog authoring, confirm
dialogs, slot screenshots, DOM menus, project `- screen` / Screen DSL.
...
| Feature | Notes |
|---------|--------|
| UI authoring | `std_ui` + optional project `ui_package` (MoonBit); see repo `docs/ui-moonbit.md` |
...
See also [Play input](./play-input) for the wait gate and skip interaction.
...
See repo `docs/host-commands.md` for full arg tables, defaults, and tween rules.

## UI bridge (author summary)

Do **not** write `- screen` in `.yuki` (compile error). Title / menu / HUD are
MoonBit (`std_ui` + optional `ui_package`) — see repo `docs/ui-moonbit.md`.
...
- [Play input](./play-input) — intents, skip hold, wait gate
- [Getting Started](./getting-started) — build and run
- Repo: `docs/ui-moonbit.md`, `docs/host-commands.md`, `docs/project-layout.md`,
